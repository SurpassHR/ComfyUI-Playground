...
submodule_name=$(awk '/workflows/ { if (match($0, /submodule "([^"]+)"/, arr)) print arr[1]; exit }' "$dot_submodule")
submodule_path=$(awk '/workflows/ && /path = / {if (match($0,/path = (.*)/,arr)) print arr[1]; exit}' "$dot_submodule")
submodule_url=$(awk '/workflows/ && /url = / {if (match($0,/url = (.*)/,arr)) print arr[1]; exit}' "$dot_submodule")
